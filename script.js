...
let cachedData = { b: 0, units: 0, energy: 100, dur: 100, level: 1, xp: 0, fish: 0, buffs: { vip: 0 }, lastBonus: 0, isAdmin: false };
let currentTab = 'main';
...
/* ==========================================================================
   [2] КОЛЕСО (НЕ ТРОГАЕМ - ОНО РАБОТАЕТ)
   ========================================================================== */
function drawWheel() {
...
async function handleSpin(cur) {
    ...
    if (balance < cost) return tg.showAlert("Мало монет!");
    isSpinning = true;
...
function renderAdminGodMode() {
    ...
    if (!list) { console.error("ID admin-user-list не найден!"); return; }
    
    list.innerHTML = `
        <div style="background:#1e293b; padding:15px; border-radius:15px; border:2px solid #ef4444; margin-bottom:15px;">
            <h4 style="color:#ef4444; margin-bottom:12px; text-transform:uppercase;">⚡ GOD MODE ACTIVE</h4>
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
                ...
                <button onclick="godCmd('give_vip', 7)" style="background:#ffd700; color:#000; padding:14px; border-radius:12px; grid-column: span 2; font-weight:950;">GIVE 7 DAYS VIP</button>
            </div>
        </div>
        <div id="raw-admin-data" style="color:#64748b; font-size:10px; font-family:monospace;">Нажмите кнопку ниже для загрузки базы...</div>
    `;
}
...
function showTab(name, el) {
    ...
    if (name === 'top') { 
        document.getElementById('leaderboard-list').innerHTML = "Загрузка ТОПа...";
        doAction('get_top'); 
    }
...
function updateTimerLogic() {
    ...
    if (goldTimer) {
        ...
        if (mins < 10) {
            goldTimer.innerText = `ЗОЛОТОЙ ЧАС АКТИВЕН!`;
            goldTimer.style.color = '#10b981';
        } else {
            goldTimer.innerText = `${59 - mins} мин до конца часа`;
...
function startFishing() {
    if (isFishingProcess || cachedData.energy < 2) return;
...
window.onload = () => { doAction('load'); setInterval(updateTimerLogic, 10000); };
